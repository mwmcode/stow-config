...
import { getFullPath, validateDirInput } from './validateDirExists.ts';
  ...
  return (await prompt([
    {
      ...
      after: async ({ configDir }, next) => {
        ...
        const configsDirFullpath = getFullPath(configDir);
    ...
    },
    {
      ...
      message: logger.prompt('Pick config file(s)'),
      options: configFiles,
      maxRows: 25,
      confirmSubmit: false,
      after: async ({ pickConfigs, destDir }, next) => {
        if (!pickConfigs?.length) {
          ...
          if (hasCreatedDestDir) {
            Deno.remove(getFullPath(destDir as string), { recursive: true });
          }
        }
        await next();
      },
